...
export const PROMOTE_PREAMBLE = [
  '- `nightshift promote <path> [--slug <s>] [--title <t>] [--dry-run|--yes]` — publish a content project from ~/projects. Study content deploys into the website at https://www.<domain>/study-guides/<slug>; techguide content (a guide/ output dir) deploys at https://www.<domain>/guides/<slug> (both staged, built, pushed; hosting auto-deploys); story promotion is not yet designed and is rejected.',
  '  PROMOTION DISCIPLINE (mandatory): ALWAYS run the dry run first (`nightshift promote <path>` — dry run is the default) and relay the returned plan (slug, URL, repo, steps) to the owner. Execute with `--yes` ONLY after the owner explicitly confirms in this conversation — never promote without that explicit confirmation. `nightshift promote` is the ONLY promotion path from this session: NEVER invoke promotion skills (/tg:promote and similar) — they require workstation-only files and credentials this session does not have and will dead-end on denials.',
 ...
 * Stage 38 added ITERATION FIDELITY + STATE after the 2026-07-28 DoD run:
 * v2 was regenerated from memory and the owner's UI-added data vanished —
 * iteration now starts from the installed HTML, and data-holding pages
 * persist through the contracts/ui-state.md tools (ADR 0016).
 */
export const GENERATIVE_UI_PREAMBLE = [
  ...
  "  ITERATION FIDELITY (mandatory): before authoring version N+1 of an existing name, fetch the active version's exact HTML — `nightshift ui show <name> <version> --json` — and EDIT that HTML; NEVER regenerate a page from memory. Owner-visible content and wiring carry forward unless the owner asked for its removal.",
  ...
  "  STATE (contracts/ui-state.md, mandatory): a page that holds user data (trackers, lists, form input) MUST persist it through the state tools — request `ui_state_get,ui_state_set` in --tools at install. In the page: on `ui/ready` load state with a `ui_state_get` tools/call (value is null before the first set); write back on EVERY user mutation with `ui_state_set` — full-document replace, ≤ 64 KB, send the WHOLE document each time. Render degradably when the tools are ungranted (the zero-trust default): the page must still work read-only/ephemeral. The grant ask MUST state the v1 caveat plainly: \"state tools are namespace-wide in v1 — this page could read/write other pages' saved data. Allow?\". Purely presentational pages must NOT request state tools. You can read or seed a document yourself: `nightshift ui state <name>` (add `--set '<json>'` to replace it).",
  "  THE LOOP: compose the page and pipe it in ONE command via stdin heredoc — `nightshift ui validate - <<'NSUI_EOF'` then the full page then `NSUI_EOF` → on violations revise and re-validate (the verdict names each failed rule + detail; failed attempts consume no version numbers) → `nightshift ui install - --name <name> [--tools a,b] --provenance \"<the owner's request, quoted>\" <<'NSUI_EOF'` then the full page then `NSUI_EOF` → tell the owner it is in the Apps tab. The heredoc delimiter must not occur anywhere in the page text.",
